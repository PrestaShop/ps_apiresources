...
class ThemeEndpointTest extends ApiTestCase
{
    ...
    {
        // These endpoints only work for 9.2+
    ...
    {
        $themeName = getenv('PS_VERSION') ? 'classic' : 'hummingbird';

        self::assertEquals($themeName, $this->getCurrentTheme());
    ...
    {
        $themeNameCurrent = getenv('PS_VERSION') ? 'classic' : 'hummingbird';
        $themeNameEnable = getenv('PS_VERSION') ? 'hummingbird' : 'classic';

        self::assertEquals($themeNameCurrent, $this->getCurrentTheme());
        $this->updateItem('/themes/' . $themeNameEnable . '/enable', [], ['theme_write'], Response::HTTP_UNPROCESSABLE_ENTITY);
    }
    ...
    {
        $themeName = getenv('PS_VERSION') ? 'classic' : 'hummingbird';
        $themeURL = getenv('PS_VERSION')
            ? 'https://github.com/PrestaShop/classic-theme/archive/refs/tags/3.1.1.zip'
            : 'https://github.com/PrestaShop/hummingbird/releases/download/v1.0.1/hummingbird.zip';
